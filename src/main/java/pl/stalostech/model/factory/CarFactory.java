...
public class CarFactory {
	...
	public List<Car> getCars() {
		...
		final List<Client> clients = clientFactory.getClients();
		final List<CarType> types = (List<CarType>) repository.findAll();

		List<Car> cars = IntStream
				.range(0, 30000).mapToObj(i -> new Car(factoryUtils.getRandomFromList(types),
						factoryUtils.randomString(), factoryUtils.dateFromYearRange(1990, 2016)))
